-- ============================================================
-- SCRIPT 6: MIGRACIÓN DE DATOS A BD_LIMON
-- ============================================================
-- Copia catálogos y datos operacionales para Sucursal Limón
-- Incluye el 40% restante de inventario y ventas
-- ============================================================

USE BD_Limon;
GO

PRINT 'Iniciando migración de datos a BD_Limon...';
GO
...
-- ============================================================
-- 5. INVENTARIO - SUCURSAL LIMÓN (40% del inventario)
-- ============================================================

-- Dividir inventario: 40% Limón
...
SELECT 
    StockItemID,
    CAST(QuantityOnHand * 0.4 AS INT) AS QuantityOnHand,
    BinLocation,
    CAST(LastStocktakeQuantity * 0.4 AS INT) AS LastStocktakeQuantity,
    LastCostPrice,
    CAST(ReorderLevel * 0.4 AS INT) AS ReorderLevel,
    CAST(TargetStockLevel * 0.4 AS INT) AS TargetStockLevel
FROM WideWorldImporters.Warehouse.StockItemHoldings;
PRINT 'StockItemHoldings: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' registros (40% inventario)';
GO
...
-- Transacciones de Stock (40% restante de las transacciones)
...
SELECT TOP 40 PERCENT
    StockItemTransactionID + 1000000 AS StockItemTransactionID, -- Offset para evitar conflictos
    StockItemID, TransactionTypeID,
    CustomerID, InvoiceID, SupplierID, PurchaseOrderID,
    TransactionOccurredWhen, Quantity
FROM WideWorldImporters.Warehouse.StockItemTransactions
ORDER BY StockItemTransactionID DESC;

SET IDENTITY_INSERT StockItemTransactions OFF;
PRINT 'StockItemTransactions: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' registros (40% transacciones)';
GO

-- ============================================================
-- 6. VENTAS - SUCURSAL LIMÓN (40% de las ventas)
-- ============================================================

-- Facturas (40% restante de las ventas)
...
PRINT 'Invoices: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' registros (40% ventas)';
GO
...
PRINT '=================================================';
PRINT 'MIGRACIÓN A BD_LIMON COMPLETADA EXITOSAMENTE';
PRINT '=================================================';
